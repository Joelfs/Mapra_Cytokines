#!/usr/bin/env python3
"""
Visualization of the XGBoost condition classifier results
(xgb_condition_classifier.py, --split stratified, from ml_models/).

Same layout as plot_rf_results.py, so the two are directly comparable:
  1. Validation accuracy per CV fold (+ mean) compared to the
     test-holdout accuracy
  2. Most important (decisive) DRVI factors (feature importances)
  3. Labeled confusion matrix (rows=true class, columns=prediction)

Additionally:
  - xgb_accuracy_summary.csv: test accuracy summary
  - xgb_condition_stratified_confusion_readable.csv: long-format table
    (true_label, predicted_label, count)

Reads exclusively already existing result files from ml_models/ (no model
re-run needed):
  - xgb_cv_fold_metrics_stratified.csv
  - xgb_feature_importances_stratified.csv
  - xgb_test_holdout_report_stratified.txt

Usage:
    conda run -n mapra_cytokines python plot_xgb_results.py
"""
...
MODELS = {
    "condition_stratified": {
        ...
        "cv_metrics": os.path.join(ML_DIR, "xgb_cv_fold_metrics_stratified.csv"),
        "importances": os.path.join(ML_DIR, "xgb_feature_importances_stratified.csv"),
        "test_report": os.path.join(ML_DIR, "xgb_test_holdout_report_stratified.txt"),
    },
    "timepoint": {
        "title": "Timepoint (TP1-TP4, ACS_sterile)",
        "cv_metrics": os.path.join(ML_DIR, "xgb_timepoint_cv_fold_metrics.csv"),
        "importances": os.path.join(ML_DIR, "xgb_timepoint_feature_importances.csv"),
        "test_report": os.path.join(ML_DIR, "xgb_timepoint_test_holdout_report.txt"),
    },
}
...
def parse_test_report(path):
    """Extracts test accuracy + confusion matrix from an xgb_*_test_holdout_report*.txt"""
    with open(path) as f:
        text = f.read()
...
def plot_model(name, cfg):
    ...
    fig.suptitle(f"XGBoost, {cfg['title']}", fontsize=14, fontweight="bold")
    ...
    out_path = os.path.join(OUT_DIR, f"xgb_{name}_overview.png")
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
    print(f"Saved: {out_path}")

    readable = confusion_to_readable(cm_df)
    readable_path = os.path.join(OUT_DIR, f"xgb_{name}_confusion_readable.csv")
...
def main():
    summary_rows = [plot_model(name, cfg) for name, cfg in MODELS.items()]
    summary = pd.DataFrame(summary_rows)
    summary_path = os.path.join(OUT_DIR, "xgb_accuracy_summary.csv")
